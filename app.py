...
def login():
    ...
    if username == "admin" and password == "secret123":
        return jsonify(
            {"success": True, "message": "Login successful", "user": username}
        ), 200
...
def update():
    ...
    if not username or new_value is None:
        return jsonify(
            {"success": False, "message": "username and new_value are required"}
        ), 400
    # Demo: pretend to update something
    return jsonify(
        {"success": True, "message": f"Updated {username} with new value: {new_value}"}
    ), 200
...
def delete():
    ...
    return jsonify({"success": True, "message": f"Deleted user: {username}"}), 200
